...
<Tabs defaultValue="layer-structure">
  <Tab value="layer-structure" label="🏗️ Layer Structure">
    ...
    **Key Patterns:**
    ...
    - **DAL Caching** - `'use cache'` in the DAL function, per read
  </Tab>
  ...
  <Tab value="data-access" label="📊 Data Access">
    **DAL (Data Access Layer):**
    ...
    // ✅ Public read: cached across requests, invalidated by tag.
    //    cache() from React still deduplicates within one request.
    ...
    // ✅ Per-user read: no cache at all — the page streams it behind <Suspense>
    ...
    **Benefits:**
    - **Caching:** `'use cache'` across requests, `cache()` within one request
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
  ...
  <div className="p-4 border rounded-lg">
    ...
    <div className="space-y-2 text-sm">
      <div>• **`'use cache'`** in DAL layer, per read</div>
      <div>• **Request deduplication** with React `cache()`</div>
      <div>• **Tag invalidation** with `updateTag` in Server Actions</div>
