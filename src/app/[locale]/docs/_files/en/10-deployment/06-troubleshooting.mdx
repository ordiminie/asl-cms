...
```bash
# Error:
Local upload error: EACCES: permission denied
```

**Solution**: the storage root must exist and be writable by the application
user, and must live outside `public/` (ADR 004).
