...
Le projet utilise `@t3-oss/env-nextjs` pour la gestion des variables d'environnement avec validation TypeScript stricte. Toutes les variables d'environnement doivent être déclarées dans [env.ts](env.ts).
...
Voir [auth.ts](src/lib/better-auth/auth.ts) et [auth-client.ts](src/lib/better-auth/auth-client.ts) pour des exemples d'utilisation avec Better Auth.
...
- Déclarer toutes les variables dans [env.ts](env.ts)
...
Voir [env.ts](env.ts) pour la configuration complète du projet avec toutes les variables d'environnement validées et typées.
