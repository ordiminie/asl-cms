...
- preferer le snake_case pour le nom des colonnes (ex : emailVerified: timestamp('email_verified'))
  [user-model.ts](src/db/models/user-model.ts)
...
[user-repository.ts](src/db/repositories/user-repository.ts)
...
[user-types.ts](src/services/types/domain/user-types.ts)
...
- Nommé le type EntityRelationData : exemple UserOrganizationData
  [organization-types.ts](src/services/types/domain/organization-types.ts)
