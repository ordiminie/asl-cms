...
s'appliquent : une story passe par le pipeline killer-saas _et_ respecte les règles
d'architecture ci-dessous.

**Piège à connaître** : `pnpm test` lance Vitest en mode _watch_ — il ne rend jamais
...
### Conventions ASL-CMS — écarts au boilerplate

Ces points **écartent ou complètent** le boilerplate. Chacun est motivé dans un ADR
(`docs/decisions/`) ; la vue d'ensemble est dans `docs/architecture.md`. Suivre le
boilerplate là où ASL-CMS s'en écarte est une erreur, pas un choix par défaut.

**Multi-tenant — la convention qui engage 42 stories** (ADR 002, ADR 003)

- Toute table métier porte `organization_id` et une policy RLS **forcée**. Chaque story
  prouve l'isolation par un test d'accès croisé entre deux tenants.
- Les repositories appellent **`getDb()`, jamais `db` directement**. Tout chemin serveur
  touchant une table métier s'exécute dans `withTenant(organizationId, ...)`.
- **Un oubli de scope ne fuite pas : il ne retourne rien.** Devant un « zéro résultat »
  inexpliqué, c'est la première hypothèse à tester.
- `withRlsBypass()` est la seule porte dérobée (SuperAdmin). Toute nouvelle occurrence
  dans un diff est un point d'arrêt de revue.
- **La RLS n'est pas testable en unitaire** : `db.ts` refuse toute connexion en test et
  les repositories sont mockés. Le test d'isolation est un **e2e Playwright**.
- Le tenant vient du **domaine appelé**, résolu côté serveur — pas dans `src/proxy.ts`,
  qui ne peut pas interroger Postgres depuis le runtime Edge.

**Rien en dur** (ADR 010)

Adresses de notification, catégories, seuils, textes, activation des relances : tout en
paramètre d'association (`organization_setting`). Les valeurs du CDCT §4.6 sont des
valeurs de **seed du premier tenant**, pas des constantes. Une valeur codée en dur est un
échec de review. Les **secrets** ne sont pas des paramètres : ils vivent dans `@/env`.

**Identité d'un membre**

Clé primaire arbitraire, **jamais l'email** (absent chez 100 des 400 propriétaires,
changeant, partagé dans un foyer) ni le **numéro de parcelle** (non unique, transmis à la
vente). Vrai pour le stockage nominatif, le rapprochement Pennylane, le dédoublonnage
d'import et l'export. La relation membre ↔ parcelle est **datée** : une vente ne transfère
pas l'historique antérieur. Ce n'est pas une FK `parcel → membre_courant`.

**Briques remplacées** — ne pas réutiliser celles du boilerplate

| Besoin            | Boilerplate                    | ASL-CMS                                | ADR |
| ----------------- | ------------------------------ | -------------------------------------- | --- |
| Fichiers          | Supabase Storage               | Disque du VPS, via le factory existant | 004 |
| Email             | Resend                         | Brevo, derrière `EmailTransport`       | 005 |
| Tâches planifiées | Inngest                        | Table `scheduled_job` + cron système   | 006 |
| Locale            | `/en` `/fr` `/es`, défaut `en` | `fr` seule, **sans préfixe d'URL**     | 008 |

next-intl est **conservé** : les libellés restent dans `messages/fr.json`, hors du code.

**Sous-systèmes retirés** (ADR 009)

Chat IA, crédits, affiliation, projets/tâches, newsletter Mailchimp sont retirés dans s01.
Ne pas les réintroduire ni s'en inspirer. Ils restent récupérables depuis `upstream/main` —
la table des chemins et des commandes est dans l'ADR 009. **Conservés** : Stripe
(facturation plateforme), `post` (base des actualités), `file`, `notification`,
`organization`, `user`, `app_settings`.

⚠️ `projects` est l'implémentation de référence citée par `rule-react-query.md` et
`rule-seed-usersroles-and-organization.md`. **s01 doit mettre ces règles à jour dans le
commit du retrait**, sinon elles pointent vers des fichiers absents.

**Deux facturations à ne jamais confondre**

Stripe = abonnement plateforme Zourite Studio ↔ association. Pennylane = factures des
membres, en **lecture seule**, derrière `MemberBillingProvider` (ADR 011). Les statuts
Pennylane sont transportés **tels quels**, jamais réduits à un booléen. Aucune donnée
bancaire ne transite par le produit : le paiement est une redirection.

**Email — le plafond est structurant**

300 envois/jour et par compte Brevo. Le budget quotidien par association est décompté par
**tout** email sortant (invitations, campagnes, relances, lancement), avec report au
lendemain. Il vit **en base**, pas en mémoire : un compteur en mémoire repart à zéro au
redémarrage et laisse dépasser le plafond.

**CMS** (ADR 007)

Une page est une **liste ordonnée de blocs typés** (Milkdown WYSIWYG + @dnd-kit), pas un
champ markdown. Les contenus répétables (actualités, fiches du bureau, analyses d'eau,
voirie) sont des modèles à **champs fixes**, pas des pages libres. Ne pas laisser
proliférer les types de blocs : un besoin non couvert est un « design system gap ».
